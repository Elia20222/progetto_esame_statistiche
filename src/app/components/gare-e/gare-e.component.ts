...
import { ServizionoGareService } from 'src/app/serviziono-gare.service';
...
export class GareEComponent implements OnInit{
  ...
  public displayColumn: string []= ['località','squadra1', 'squadra2', 'punteggioresquadra1', 'punteggioresquadra2']
  ...
  constructor(
    ...
    public ServizionoGare: ServizionoGareService,
  ...
  getlistasquadre(){
      const Idgare = this.route.snapshot.paramMap.get('id')
      this.dataSource = new MatTableDataSource (this.ServizionoGare.getfakegare().games)

    //   // this.Apiservice.getGare(Idgare).subscribe(res=>{
    //   this.Squadra = res
    //   this.dataSource = new MatTableDataSource(res.competitions)
